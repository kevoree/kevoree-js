...
const instanceResolver = require('../util/instance-resolver');
...
module.exports = (model, expressions, stmt, opts) => {
  const setLeftPart = stmt.children[0];
  let paramValPart, targetNodes;
  ...
  const leftPart = expressions[setLeftPart.type](model, expressions, setLeftPart, opts);
  ...
  const paramPart = setLeftPart.children.splice(-1)[0];
  const instancesPath = expressions[setLeftPart.type](model, expressions, setLeftPart, opts);
  const instances = instanceResolver(model, expressions, setLeftPart, opts);
  ...
  const paramName = expressions[paramPart.type](model, expressions, paramPart, opts);
  if (paramName === '*') {
    throw new KevScriptError('You must not use wildcard (*) for parameter name. Set failed', setLeftPart.pos);
  }
  const paramVal = expressions[paramValPart.type](model, expressions, paramValPart, opts);
  ...
  if (targetNodes) {
    ...
    targetNodes.forEach((node) => {
      instances.forEach((instance) => {
        const fDics = instance.select('fragmentDictionary[' + node.name + ']').array;
        if (fDics.length === 0) {
          ...
          if (instance.typeDefinition.dictionaryType) {
            const att = instance.typeDefinition.dictionaryType.findAttributesByID(paramName);
            if (att) {
              if (att.fragmentDependant) {
                fDics.forEach((dictionary) => {
                  let dVal = dictionary.findValuesByID(paramName);
                  if (!dVal) {
      ...
      if (instance.typeDefinition.dictionaryType) {
        ...
        if (att) {
          if (att.fragmentDependant) {
            ...
            let dVal = instance.dictionary.findValuesByID(paramName);
            if (!dVal) {
              dVal = factory.createValue();
              dVal.name = paramName;

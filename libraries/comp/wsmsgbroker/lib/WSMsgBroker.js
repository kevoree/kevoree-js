...
var WSMsgBroker = AbstractComponent.extend({
    ...
    start: function (done) {
        ...
        if (port) {
            ...
            this.log.info('"'+this.getName()+'" listen on 0.0.0.0:'+port+path);

            // register some event listeners on it
            this.server.on('error', function (err) {
                this.log.error('"'+this.getName()+'" broker error: '+err.message);
            }.bind(this));

            this.server.on('warn', function (err) {
                this.log.warn('"'+this.getName()+'" broker warning: '+err.message);
            }.bind(this));

            this.server.on('registered', function (id) {
                this.log.info('"'+this.getName()+'" '+chalk.green('+')+' '+id);
            }.bind(this));

            this.server.on('unregistered', function (id) {
                this.log.info('"'+this.getName()+'" '+chalk.yellow('-')+' '+id);
            }.bind(this));
